...
contract AssetManagement {
    ...
    mapping(uint256 => Asset)  assets;//使用映射存储资产，其中键是资产 ID，值是资产的详细信息。
...
//请求资产转让的函数，生成转让请求并触发 AssetTransferRequested 事件。
    function requestAssetTransfer(address initiator,uint256 _assetId, address _to) public {
        ...
        Request memory r=Request(_assetId,asset.name,asset.ownerAddress,_to);
...
//批准资产转让的函数，仅允许管理员调用，更新资产拥有者并触发相应事件。
    function approveAssetTransfer(address initiator, uint256 _assetId) public  returns(bool) {
        ...
        if (personAllAsset[oldOwner].length > 1) {
            for (uint i = _assetId; i < personAllAsset[oldOwner].length - 1; i++) {
                personAllAsset[oldOwner][i] = personAllAsset[oldOwner][i+1];
            }
            delete personAllAsset[oldOwner][personAllAsset[oldOwner].length - 1];
