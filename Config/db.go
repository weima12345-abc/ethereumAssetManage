...
func init() {
	...
	db, err = sql.Open("mysql", "root:123456@tcp(127.0.0.1:3306)/finalwork") // 设置连接数据库的参数
	if err != nil {
		panic(err)
	}
	err = db.Ping() //连接数据库
...
func UserLogin(username string, password string) int {
	rows, err := db.Query("SELECT password FROM user where username=?", username) //通过账户名查询密码
	if err != nil {
		fmt.Println("查询失败")
	}
...
func UserRegister(c *gin.Context, username string, password string) int {
	...
	var a int
	rows, err := db.Query("SELECT  *  from user where username=?", username) //查询superoot内所有信息
	fmt.Println(err)
	if err != nil {
	...
	if username != u.name {
		...
		results, err := db.Exec("insert into user(username,password) values(?,?)", username, password) //执行插入sql语句
		if err != nil {
...
func UserChange(username string, password string) (string, string) {
	...
	results, err := db.Query("update  user set username=?,password=? where username=123", username, password) //查询superoot内所有信息
